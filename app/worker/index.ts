...
interface ScoreRow {
  name: string;
  score: number;
  /** The Tier the run was flown at, 1..MARK_MAX. 0 means "untiered": a client
   *  that predates tier boards and sent no mark at all. Never a real Tier. */
  mark: number;
...
async function handleApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (url.pathname === "/api/scores" && request.method === "GET") {
    ...
    const mark = raw === null ? null : clampInt(raw, 1, 0, MARK_MAX);
  ...
  if (url.pathname === "/api/scores" && request.method === "POST") {
    ...
    // 0 is the floor on purpose: a client that predates tier boards sends no
    // mark, and 0 is where it lands. Rejecting it would 400 every shipped
    // store build the moment this deploys.
    const mark = clampInt(body.mark, 0, 0, MARK_MAX);
