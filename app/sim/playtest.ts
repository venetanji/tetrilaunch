...
// ---------------------------------------------------------------------------
console.log("\n3. CLOCK");
const wins = bays.filter((b) => b.result === "won");
if (wins.length) {
  ...
  console.log("   no bays won yet");
}
const byReason = new Map<string, number>();
for (const b of bays.filter((x) => x.result === "lost")) {
  byReason.set(b.reason ?? "?", (byReason.get(b.reason ?? "?") ?? 0) + 1);
}
console.log(`   losses: ${[...byReason].map(([r, n]) => `${r} x${n}`).join(", ") || "none"}`);

// ---------------------------------------------------------------------------
// 4. Bankroll pressure — how close to broke, and when.
// ---------------------------------------------------------------------------
console.log("\n4. BANKROLL");
const mins = bays.map((b) => {
  const lowest = b.funds.length ? Math.min(...b.funds.map((f) => f.v)) : b.endScore;
  return { bay: b.bay, lowest, cost: b.launchCost, shotsLeft: Math.floor(lowest / Math.max(1, b.launchCost)) };
});
...
// ---------------------------------------------------------------------------
console.log("\nPer bay:");
console.log("  bay mark  result  reason   secs/limit  lines  shots  s/line  medWait  end/target");
for (const b of bays) {
  ...
  console.log(
    ...
    `${b.secs.toFixed(0)}/${b.timeLimitSec}`.padStart(12) +
