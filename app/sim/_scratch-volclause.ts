 ...
 *   CLAUSE=powder-keg npx tsx sim/_scratch-volclause.ts   # the SHIPPED clause
 *
 * CLAUSE swaps the written share for a real FinalDef's own apply(), which is
 * how a drafted number is re-checked against the code that ships it rather
 * than against the line in this file that proposed it. The share column then
 * prints what the clause actually dealt.
 *
...
import { finalById } from "../src/game/finals";
...
const CLAUSE = process.env.CLAUSE ? finalById(process.env.CLAUSE) : undefined;
if (process.env.CLAUSE && !CLAUSE) throw new Error(`no clause ${process.env.CLAUSE}`);
...
for (const share of SHARES) {
  for (const tier of TIERS) {
    for (const [pname, spec] of [["naive", naiveStrategy], ["cushion", cushionStrategy]] as const) {
      ...
      // What the belt ACTUALLY carried, read back off the last bay built: under
      // CLAUSE the share is the clause's business, not this file's.
      let dealt = share;
      for (const seed of seeds) {
        ...
        // or, under CLAUSE, the shipped clause's own apply() instead of the
        // write, so the row prices the code rather than the proposal.
        if (CLAUSE) CLAUSE.apply(flown);
        else flown.materialMix = { ...flown.materialMix, volatile: share };
        ...
        dealt = flown.materialMix.volatile;
      ...
      console.log(
        `| ${(100 * dealt).toFixed(0)}% | ${tier === 0 ? "none" : `t${tier}`} | ${pname} `
