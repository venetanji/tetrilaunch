...
export function generateContract(
  ...
  forceSetpiece = false,
  /** DIFFICULTY BUDGET OVERRIDE — the tier's own, unless a caller states
   *  otherwise. One caller does (schoolContract below), and it is the reason
   *  this is a parameter rather than a fork of the generator: the school's card
   *  has to be the same Contract every other card is, generated by the same
   *  function against the same feasibility model, and differ in exactly one
   *  authored number. A parallel builder would be a second answer to "what is a
   *  tier-1 lines Contract" the day the model moves. */
  ...
  const rng = mulberry32(seed + slot * 7919);
  let budget = budgetOverride ?? budgetForTier(tier);
...
/* ---------------------------------------------------------------------------
 * THE SCHOOL'S CONTRACT — one card, the same one, for everybody, forever.
 *
 * The ground floor's fifth step (meta.ts's schoolLadder). While the school is
 * unfinished the board deals THIS and nothing else: the daily three, the
 * pattern slot and the Set Piece alternation all arrive at graduation, with the
 * rest of the game.
 *
 * WHY ONE CARD AND NOT THREE. A board is a comparison — three offers in three
 * kinds, and the whole reason the pattern slot is fixed rather than rolled is
 * so the row is scannable. A player who has just flown four lessons has nothing
 * to scan with: they have never seen a launch budget, a manifest or a timed
 * crush, and the step in front of them is not "choose a Contract" but "clear
 * one, and spend what it pays". The owner's call is exactly that — "make it a
 * single contract for this school floor" — and a single card is also the only
 * shape that lets the board's own header state what a Contract IS without
 * arguing with two cards that are something else.
 *
 * WHY IT IS FIXED. The daily board is a function of the DATE, which makes it
 * the one surface in the school whose difficulty depends on when you installed
 * the game. Every lesson bay is seeded (school.ts's lessonSeed) precisely so a
 * teaching bay is the same bay on the second attempt; a taught Contract has the
 * same claim on being repeatable, and a fixed seed is how this file has always
 * spelled that.
 *
 * WHY IT IS CLEAN. `budgetForTier(1)` is 2 and every tier-1 complication costs
 * exactly 2, so an ordinary tier-1 card always spends its whole budget on one
 * of wind, dominoes or a tight launch budget — measured across eleven seeds, all
 * eleven drew crosswind. Tier 1 bay 1 has `windMax` 0 (level.ts), no lesson
 * mentions weather, and the graduation flight two rungs later is that same
 * windless bay. So the school's card is generated at budget ZERO: the first
 * Contract anybody meets is the mode with nothing on top of it, which is what
 * the header is about to promise them.
 *
 * MEASURED, over ten bot seeds through the real levelForContract:
 *
 *   Quota Run, 3 lines / 12 launches   aim 9/10   lob 7/10   lob-rot 7/10
 *   ...the same card at budget 2       aim 8/10   lob 6/10   lob-rot 8/10
 *
 * i.e. the crosswind the tier's own budget always buys costs the pilots about
 * nothing and teaches a word nothing on the ladder has said. What the same
 * sweep says about the ALTERNATIVE — dealing the ordinary three-card board
 * here — is the sharper number: today's tier-1 daily puts a PATTERN Contract in
 * its fixed third slot, and the calibration bot clears that 2/10 (`lob` 1/10,
 * `lob-rot` 0/10). A board where one card in three is the hardest thing in the
 * mode is not a first Contract. Bots never retry and a Contract is free to, so
 * these are floors rather than forecasts.
 *
 * ITS ID IS NOT A DAILY ID, and that is worth two things. `claimedContracts`
 * keys off the id, so the clear is banked forever rather than until midnight —
 * and `claimedContractsOnDay` counts by the day's seed PREFIX, so clearing the
 * school's card does not spend one of a free account's three daily clears. The
 * school does not eat the day it is played on.
 * ------------------------------------------------------------------------- */

/** The school's Contract is filed at Tier 1 — which is what makes its clear
 *  bank a tier-1 milestone (meta.ts's recordContractClear compares the card's
 *  tier against markUnlocked) and therefore what pays for step 6. */
export const SCHOOL_CONTRACT_TIER = 1;

/** Its seed. Off the lesson block (school.ts's lessonSeed is `0x5c400 + index`)
 *  and deliberately clear of it, so no lesson and no Contract can ever collide
 *  on an id.
 *
 *  CHOSEN FOR THE SHORTEST GOAL THE GENERATOR DEALS. A tier-1 card asks 3-5
 *  lines off `3 + floor(rng*3)`, and this seed is a 3 — twelve launches for
 *  three rows, which is the whole ask. The seeds in the block that deal a 3
 *  measured identically (a lines Contract has no queue to order), so the pick
 *  among them is the name on the card. */
export const SCHOOL_CONTRACT_SEED = 0x5c502;

/** No complications — see the note above. Named rather than typed as a bare 0
 *  at the call site, because a play pass will edit this before it edits
...
/**
 * Is this id the school's own card?
 *
 * ANSWERED BY GENERATING THE CARD, not by re-spelling its id. The id format is
 * `${seed}-${tier}-${slot}` in three places in this file and the Contract type
 * says why that is not a fourth: "reading it back out of a string is how an id
 * format change becomes a silent behaviour change". A caller that needs to
 * recognise this one card therefore asks the generator, which costs one
 * mulberry32 walk (schoolContract is pure and memo-free by construction).
 *
 * It exists because meta.ts needs the question: the school's card is the only
 * Contract in the game whose clear can pay TWICE (meta.ts's schoolRescueOwed —
 * the ground floor's one rescue), and a rule that identified it by tier or by
 * slot would hand that exception to every tier-1 card in slot 0.
 */
export function isSchoolContract(id: string): boolean {
  return id === schoolContract().id;
}

/** The school's whole board. A one-element array rather than a Contract,
 *  because every caller downstream — the screen, the card index main.ts routes
 *  a tap through, the "next uncleared" walk on the end card — is written
 *  against a board and a board of one is the honest way to say so. */
export function schoolBoard(): Contract[] {
