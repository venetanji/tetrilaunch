...
export function hudHTML(opts: {
  ...
  contract?: {
    ...
    conditions: string;
    /** The Contract's OWN tier (Contract.tier) — the bay's tier, which is not
     *  the player's once they have climbed past the board entry they are
    ...
    <div class="plant">
      ...
      <div class="plant__body">
        ...
        ${
          ...
          // static while the bay plays (above). It DOES change across renders
          // — a clear that banks the milestone advances the count, and the
          // clear that completes a tier drops the row to Practice — but both
          // land via contract-end's own fresh hudHTML() render, not a live
          // patch, so main.ts still never looks the id up.
          contract
            ? `<div class="pl-tier"><span class="lbl">Tier ${contract.progress?.tier ?? contract.tier}</span><b id="hud-tier">${
                ...
                // rather than the player's — a tier-6 board entry replayed at
                // tier 7 is still a tier-6 bay, and naming the player's tier
                // beside "practice" would just raise the same wrong number the
                // row is being fixed for. The row stays mounted either way:
                // the layout is one line whichever it says.
                contract.progress
