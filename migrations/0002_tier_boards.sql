...
--   1..10   the Deep Run board for that Tier
--   -1      Tier S (lib/api.ts's BOARD_SANDBOX) — not a rung, and it must never
--           share a list with one. Three branches independently reached for the
--           `level` column as a board key: #88 for the Tier, #90 for the mode.
--           Through one column those collide (#88's Tier 2 and #90's Tier S are
--           both `level = 2`), which would have filed sandbox scores on the
--           Tier 2 leaderboard. One column with a domain that has room for both
--           is what stops that.
--   0       untiered, where a pre-tier-boards client lands
